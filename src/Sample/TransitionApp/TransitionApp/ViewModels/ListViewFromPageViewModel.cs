...
{
    public class ListViewFromPageViewModel : ViewModelBase
    {
        ...
        public ListViewFromPageViewModel(INavigationService navigationService) : base(navigationService)
        {
            NavigateDogCommand = new DelegateCommand<DogModel>(async (selectedDog) =>
            {
               ...
               await navigationService.NavigateAsync($"{nameof(ListViewToPage)}",navParam);
            });
